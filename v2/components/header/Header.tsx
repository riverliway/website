...
export const Header: React.FC<HeaderProps> = props => {
  ...
  return (
    <nav
      className='fixed w-full flex justify-center border-b border-gray-600 backdrop-blur-md'
      style={{ backgroundColor: props.theme === 'dark' ? '#2300297f' : '#4da5e37f' }}
    >
      <div className='w-full md:w-9/12 flex flex-row justify-between align-center p-2'>
        {!isSmallPhone && (
          <a className='flex flex-row justify-center align-center cursor-pointer' href='/?self=1'>
            <Signature shouldInitialAnimate={shouldAnimate} />
          </a>
        )}
...
const shouldAnimateHeader = (): boolean => {
  ...
  if (params.has('self') && params.get('self') === '1') {
    // To clear the query params
